//ATENCAO! CODIGO ALTERADO E DIFERENTE DA LIBRARY ORIGINAL!
//MODIFICADO POR [ANDERSON XAVIER] EM [22/05/2017]
...
*/
//ANDERSON XAVIER 22/05/2017

var findwindow_PrincipalDiv = 'findwindow';
var fwtext = 'fwtext';
var Dynamicfind = false;
...
var find_root_node = 'divTextp5'; //null; // Leave as null to search entire doc or put id of div to search (ex: 'content'). Ver 5.0a - 7/18/2014
...
//Anderson Xavier
function setfindwindow(dom_element_id) {
    findwindow_PrincipalDiv = dom_element_id;
}


function setfindto(dom_element_id) {
    find_root_node = dom_element_id;
}

function setfwtext(dom_element_id) {
    fwtext = dom_element_id;
}
...
function findit() {
    ...
    if (find_text.toLowerCase() == document.getElementById(fwtext).value.toLowerCase() &&
        ...
        if (highlights.length > 0) // if we found occurences
        {
            ...
            find_msg.innerHTML = "&nbsp;<b>0 de 0</b>"; // ver 5.1 - 10/17/2014 - changed from "Not Found"
            find_pointer = -1;
        }
    }
...
function findnext() {
    ...
    find_msg.innerHTML = display_find + " de " + highlights.length;
...
function resettext() {
    if (find_text.toLowerCase() != document.getElementById(fwtext).value.toLowerCase())
        unhighlight(); // Remove highlights of any previous fi'fwtext'nds
...
var scrollLeft, scrollTop, scrollBottom, scrollRight;

function getActualPosition(field) {
    scrollLeft = document.body.scrollLeft || document.documentElement.scrollLeft;
    scrollTop = document.body.scrollTop || document.documentElement.scrollTop;
    scrollBottom = (window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) + scrollTop;
    scrollRight = (window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) + scrollLeft;
}
...
function scrollToPosition(field) {
    if (field) {
        var _thisElement = jQuery(field);

        jQuery('.profileContainer').animate({
            scrollTop: _thisElement.position().top
        }, 100);
    }
}
...
function create_div(dleft, dtop, dwidth, dheight) {
    ...
    if (document.getElementById(findwindow_PrincipalDiv)) {
        findwindow = document.getElementById(findwindow_PrincipalDiv);
...
// Descomentar aqui em caso te teste!
//**************************
...
window.onload = function () {
    find_msg = document.getElementById('find_msg');
};

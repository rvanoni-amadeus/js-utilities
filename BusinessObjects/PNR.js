...
var loadScripts = (function() {
    ...
    function addScript() {
        ...
        loadArrayOfScripts(scriptURLArr, webSdkPlatform + "src/app/assets/js/BusinessObjects/");
        loadArrayOfScripts(scriptURLArrFromUtils, webSdkPlatform + "src/app/assets/js/Util/");
    }
...
var PNR = (function() {
    ...
    var parsePNRRespone = function(status) {
        ...
        if (!res) {
            ...
            var promiseVar = SendObj.SendCryptic("rt");
            promiseVar.then(function(crypticRespData) {
                var checkGroupResponse = crypticRespData.Response.split(/\n/);
    ...
    function sendCrypticForPNR(self, mainData, status, pnrType, crypticData) {
        ...
        if (noACtivePNR === null || noACtivePNR === undefined) {
            ...
            if (pnrType === "groupPNR") {
                ...
                var promiseVarSec = SendObjSec.SendCryptic("rtn");
                promiseVarSec.then(function(crypticGroupRespData) {
                    if (crypticGroupRespData.Response.slice(-2) === ")>") {
                ...
                if (crypticData.Response.slice(-2) === ")>") {
                    var HelperCrypticObj = new helperCryptic();
                    var varPromise = HelperCrypticObj.GetCrypticResponse(crypticData.Response);
    ...
    function assignTextProperty(crypticCmdResponse, self, itinerayObj) {
        var crypticResponse;
        crypticResponse = crypticCmdResponse.Response ? crypticCmdResponse.Response.split(/\n/) : crypticCmdResponse.split(/\n/);
